...
const dataList = list.map((file) => {
  const data = fs.readFileSync(file.path, 'utf8');
  return parseInfo(file, data.split('---')[1]);
}).filter((row) => row !== null).filter((row) => row.public !== 'false').sort((a, b) => a.fileName.toLowerCase().localeCompare(b.fileName.toLowerCase()));
...
for (tag in tagMap) {
  tagMap[tag].sort((a, b) => a.fileName.toLowerCase().localeCompare(b.fileName.toLowerCase()));
}

saveTagMap(tagMap);

const pageMap = {};
dataList.sort((a, b) => a.url.toLowerCase().localeCompare(b.url.toLowerCase())).forEach((page) => {
...
function saveTagCount(tagMap) {
  ...
  fs.writeFile('./_data/tagCount.yml', YAML.stringify(sortedList), (err) => {
    if (err) {
      return console.log(err);
    }
...
function parseInfo(file, info) {
  ...
  const { mtime } = fs.statSync(file.path);
  obj.modified = mtime;
